...
func (h *Handler) Login(c *gin.Context) {
	...
	if user == nil { // Email not found
		_ = c.AbortWithError(http.StatusBadRequest, errors.New("incorrect email address or password"))
		return
	}
...
func (h *Handler) Signup(c *gin.Context) {
	...
	if user != nil {
		_ = c.AbortWithError(http.StatusBadRequest, errors.New("email address already taken"))
		return
	}
...
func (h *Handler) ChangePassword(c *gin.Context) {
	...
	if user == nil {
		_ = c.AbortWithError(http.StatusForbidden, errors.New("invalid credentials"))
		return
	}
...
func (h *Handler) UploadFile(c *gin.Context) {
	...
	if storageMetrics.SizeUsed+file.Size > storageMetrics.SizeAllowed {
		_ = c.AbortWithError(http.StatusRequestEntityTooLarge, errors.New("file upload exceeds account size limit"))
		return
	}
...
func (h *Handler) DownloadFile(c *gin.Context) {
	...
	if !rows.Next() {
		_ = c.AbortWithError(http.StatusNotFound, errors.New("file not found"))
		return
	}
	defer rows.Close()
...
func (h *Handler) GetLink(c *gin.Context) {
	...
	if !found {
		_ = c.AbortWithError(http.StatusNotFound, errors.New("link not found"))
		return
	}
...
func (h *Handler) RequestResetPassword(c *gin.Context) {
	...
	if err != nil {
		...
		_ = c.AbortWithError(http.StatusInternalServerError, errors.New("failed to get user from database"))
		return
	}
	if user == nil {
	...
	if err != nil {
		...
		_ = c.AbortWithError(http.StatusUnauthorized, errors.New("failed to save reset-code into database"))
		return
	}
...
func (h *Handler) ResetPassword(c *gin.Context) {
	...
	if dbPR == nil {
		_ = c.AbortWithError(http.StatusBadRequest, errors.New("reset-code invalid"))
		return
	}
	...
	if time.Since(dbPR.CreatedAt) > time.Minute*10 {
		_ = c.AbortWithError(http.StatusBadRequest, errors.New("reset-code expired"))
		return
	}
	// Validate password strength
	if err = ValidatePassword(req.NewPassword); err != nil {
